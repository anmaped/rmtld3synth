...
module Sys = struct
    ...
    match s with
    [] -> [cm]
    | a::x -> if a = ' ' && b mod 2 = 0 then [cm]@(console_arg_split x "" b )
              else console_arg_split x (cm^(if a = '\"' then "" else (String.make 1 a))) (if a = '\"' then b+1 else b)

    let getenv x = List.assoc x Url.Current.arguments
    let argv = Array.of_list (console_arg_split (String.explode (getenv "argv")) "" 0 )
...
let _ =
Sys_js.create_file ~name:"/static/config/default" ~content:"(buffer_size 100)
