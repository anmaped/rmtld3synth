...
Lets overview a simple monitoring case generation. Before, we begin with the description of the synthesis settings, and then, we describe the interface with the SUO. In this phase, we use a simple test case that is automatically generated by our synthesis tool as a demo.

### Overview of the configuration settings

Settings for RMTLD3 synthesis tool are described using the syntax `(<setting_id> <bool_type | integer_type | string_type>)`, where '|' indicates the condition or, and `setting_id` the available setting parameter of type string.
...
`gen_tests` sets the automatic generations of test cases (to be used as a demo in the next illustration).
...
where \f$\alpha\in \realnumber\f$, \f$x \in \mathcal{V}\f$ is a logic
...
\f$\gamma \in \nonnegativerealnumber\f$, and the meaning of \f$\eta_1 < \eta_2, \varphi_1 \lor \varphi_2, \neg \varphi, \exists x \, \varphi\f$ is defined as usual.
