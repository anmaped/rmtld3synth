...
let rmtld3_unit_test_case_generation trace formula computed_value
    cpp11_compute helper cluster_name n =
  let id =
        ...
        ( ( a ^ "\ttmp_x = event_t("
          ^ ( find_proposition_hashtbl p helper
            ...
            (*Printf.printf "Proposition %s is not found.\n" d ; raise
              (Failure "rmtld3_synth_test: proposition is missing.")*) )
          ^ ","
          ^ string_of_int (int_of_float t1)
          ^ ");\n\t__buffer_" ^ cluster_name ^ ".push(tmp_x);\n"
        , count + 1 ) )
      ( monitor_eval id ^ "\n\ninline static bool __unit_test_"
...
let rmtld3_unit_test_generation cluster_name cpp11_compute helper =
  ...
  let pass_test_n expected_value lb trace formula n =
    ...
    (*for rep = 1 to d do eval (k, t_u, 0.) formula; done;*)
    ...
    if t_value = expected_value then (
      ...
      call_code :=
        !call_code ^ "// " ^ lb ^ "\n"
        ^ rmtld3_unit_test_case_generation trace formula t_value
            cpp11_compute helper cluster_name n ;
  ...
  if get_setting_string "gen_unit_tests" helper = "true" then (
    ...
    pass_test True "int 5 A < 3.0(0)1  " test1_trace
      (LessThan
         (Duration (Constant 5., Prop "A"), Constant (3. +. epsilon_float))
      ) ;
    ...
    (* 2*(x-7)+2*(x-6)+2*(x-5)+2*(x-4)+2*(x-3)+2*(x-2)+2*(x-1)+2*x+x * is
       simplified to 17x-56 where x is the length of the trace *)
    ...
    pass_test False
      ...
      (Until
         ...
         , Until
             ...
             , Until (10., Prop "A", Prop "B") )
         , Prop "STAR" ) )
    (* * NEW test [TODO] (((3.528682 < int[int[0.643269 ] (E ) ] ((E or E ))
       ) or ((E U_3.204527 E ) o r (int[2.963296 ] (E ) < 3.456293 )))
       U_4.239142 ((int[0.333695 ] (B ) < int[2 .105323 ] (A ) ) U_2.887519
       ((int[3.716714 ] (E ) < 3.871726 ) U_1.413040 (E o r E ))))Duration
       44.778000s Metrics: Cardinality: 101 Measure(temporal operators): 4
  ...
  if get_setting_string "gen_paper_results" helper = "true" then (
    ...
    for trace_size = 1 to 3 do
      ...
      let new_trace =
        List.rev
          (strategic_uniform_trace 0.
             (List.nth lst (trace_size - 1) - 1)
             1. [] )
      in
    ...
    (*for trace_size = 1 to 3 do
      ...
      let new_trace3 = repeat_trace ( (List.nth lst (trace_size-1))/8)
      sample_trace [] in pass_test_n True ("A->eventually "^(string_of_int
      (List.nth lst (trace_size-1)))^". B") new_trace3 ( mimplies (Prop("A"))
  ...
  let usecase1_formula =
    m_or_fold
      (List.rev
         ...
         ; Prop "A" ] )
  in
  (*let oc = open_out "formula_out" in Sexp.output_hum_indent 2 oc
    (sexp_of_formula usecase1_formula); close_out oc;*)
  let trc =
    [ ("B", 0.)
    ; ("A", 1.)
    ; ("A", 2.)
  ...
  in
  pass_test Unknown
    (string_of_rmtld_fm usecase1_formula)
    trc usecase1_formula ;
  ...
  output_string oc
    (beautify_cpp_code
       ...
       ^ !call_code ^ "\ninline static void __run_unit_tests (void) {"
       ^ !call_list ^ "\n};" ) ) ;
  close_out oc ;
