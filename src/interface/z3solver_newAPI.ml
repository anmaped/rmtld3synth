...
let solve_ ctx exp =
  let tactic_set =
    and_then ctx (mk_tactic ctx "qe") (mk_tactic ctx "smt") []
  in
...
let get_scheduler ctx model helper =
  ...
  let seq_model =
    try
      let seq =
        List.find
          (fun el -> get_string (get_name el) = "trc")
          (get_decls model)
      in
      ...
      verb (fun _ ->
          print_endline
            ("Sequence: " ^ Z3.AST.to_string (ast_of_expr seq_model)) ) ;
      seq_model
    with _ -> failwith "No sequence available."
  in
  let get_array_interp_from_exp exp i : string =
    (* check if array is a quantifier/lambda or a constant array *)
    if not (Z3.AST.is_quantifier (ast_of_expr exp)) then
      if not (Z3.Z3Array.is_constant_array exp) then
        failwith
          ( "trc is not a quantifier/lambda nor a constant array: '"
          ^ Z3.AST.to_string (ast_of_expr exp)
          ^ " '"
          ^ string_of_int
              (Z3enums.int_of_ast_kind (Z3.AST.get_ast_kind (ast_of_expr exp)))
          ^ "'" ) ;
    if Z3.Z3Array.is_constant_array exp then
      Z3.AST.to_string (ast_of_expr (List.hd (Z3.Expr.get_args exp)))
    else
      let body =
