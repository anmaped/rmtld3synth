...
let get_scheduler ctx model helper =
  ...
  let interp =
    match get_func_interp model trace with
    | Some x -> x
    | None -> raise (Failure "Function interpret is not available.")
  in
  verb (fun _ ->
      print_endline ("N interp: " ^ string_of_int (get_num_entries interp)));
  ...
  let size_fun_interp =
    ...
    | None ->
        raise
          (Failure
             "Interpretation of the function (size_fun_interp) is not \
              available.")
  in
  ...
  let size =
    if Z3.Expr.to_string size_fun_interp = "(- 1)" then 2
    else int_of_string (Z3.Expr.to_string size_fun_interp) + 2
  in
