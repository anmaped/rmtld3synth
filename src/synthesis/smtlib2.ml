        ...
        (Term.forall
           ...
           (f_implies
              (f_not
                 (f_and
                    (f_and
                       (f_leq (f_const_term "0") (f_const_term "x"))
                       (f_leq (f_const_term "x")
                          (f_const_term
                             (string_of_int !recursive_unrolling_depth) ) ) )
                    (f_and
                       (f_leq (f_const_term "0") (f_const_term "i"))
...
let synth_tm_constant value helper =
  empty_body
    (Term.apply (f_const_term "dsome")
       [Term.const (Id.mk Id.Term (string_of_int (int_of_float value)))] )
...
let synth_tm_variable name helper =
  ...
  empty_body
    (Term.apply (f_const_term "dsome") [Term.const (Id.mk Id.Term name)])
         ...
         ((f_and
             (f_less
                (Term.apply (f_const_term "select")
                   [f_const_sort "trc_time"; f_const_sort "i"] )
                (f_const_sort (string_of_int 100)) ) )
            (f_less (f_const_sort "0")
               (Term.apply (f_const_term "select")
                  [f_const_sort "trc_time"; f_const_sort "i"] ) ) ) ) ) ;
  let duration id fm =
    let l_lst = ref [] in
                   ...
                   (f_sum
                      ...
                      (f_times
                         (Term.apply
                            ...
                            [ f_const_term "trc"
                            ; f_const_term (string_of_int x) ] )
                         ( if !assume_unary_sequence then f_const_term "1"
                           else
                             Term.apply (f_const_term "select")
...
let synth_tm_plus cmptr1 cmptr2 helper =
  ( Term.apply
      (Term.const (Id.mk Id.Term "dsome"))
      [ f_sum
          (Term.apply (Term.const (Id.mk Id.Term "dval")) [fst cmptr1])
          (Term.apply (Term.const (Id.mk Id.Term "dval")) [fst cmptr2]) ]
...
let synth_smtlib_common_types () =
  ...
  define_sort lst
    (Id.mk Id.Sort "Proptype")
    []
    (Term.const (Id.mk Id.Sort "Int")) ;
  define_sort lst (Id.mk Id.Sort "Time") []
    (Term.const (Id.mk Id.Sort "Int")) ;
  define_sort lst (Id.mk Id.Sort "Index") []
    (Term.const (Id.mk Id.Sort "Int")) ;
  ...
  define_sort lst (Id.mk Id.Sort "Trace_") []
    (Term.apply
       (Term.const (Id.mk Id.Sort "Array"))
       [Term.const (Id.mk Id.Sort "Index"); Term.const (Id.mk Id.Sort "Real")] ) ;
...
let synth_smtlib_common_trace () =
  ...
  declare_const lst (Id.mk Id.Term "trc")
    (Term.const (Id.mk Id.Sort "Trace")) ;
  if not !assume_unary_sequence then
    declare_const lst
      (Id.mk Id.Term "trc_time")
      (Term.const (Id.mk Id.Sort "Trace_")) ;
  declare_const lst
    (Id.mk Id.Term "trc_size")
    (Term.const (Id.mk Id.Sort "Time")) ;
  if not !assume_unary_sequence then (
    declare_fun lst
      (* id *)
    ...
    if !recursive_unrolling then (
      ...
      assert_ lst
        (f_equal
           (Term.apply (f_const_term "mapt") [f_const_term "(- 1)"])
           (f_const_term "0") )
        ...
        (Term.forall
           ...
           (ite
              ...
              (f_equal
                 ...
                 (f_sum
                    (Term.apply (f_const_term "mapt")
                       [f_minus (f_const_term "i") (f_const_term "1")] )
                    (Term.apply
                       (Term.const (Id.mk Id.Term "select"))
                       [ Term.const (Id.mk Id.Term "trc_time")
                       ; Term.const (Id.mk Id.Term "i") ] ) ) )
