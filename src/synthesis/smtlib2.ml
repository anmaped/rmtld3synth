...
let synth_tm_variable name helper =
  if not (SS.exists (fun s -> s = name) !free_variables_set) then (
    ...
    declare_const lst (Id.mk Id.Term name) (Term.const (Id.mk Id.Sort "Real")));
  empty_body
...
let synth_fm_aw_eq gamma sf1 helper =
  synth_fm_not
    (synth_fm_uless gamma (empty_body f_tvtrue) (synth_fm_not sf1 helper) helper)
    helper
...
let synth_smtlib_common_trace () =
  ...
  if not !assume_unary_sequence then
    declare_const lst (Id.mk Id.Term "trc_time")
