...
let fm_to_fm_disj_ex_map (fm : fm) : map_of_fm_disj_ex =
  ...
  let fld : 'a -> idx_ct -> 'b -> 'b =
   fun key value b ->
    match value with
    | KUntil (pval, fm1, fm2) ->
        let tag : idx_ct_fm_disj_ex =
          KUntil
            ( pval
            , fm_disj_to_fm_disj_ex (fm_disj_of_fm fm1)
            , fm_disj_to_fm_disj_ex (fm_disj_of_fm fm2) )
        in
        Fm_container.add (Sexp.to_string (sexp_of_kmap (KProp key))) tag b
        (* fm_disj_of_fm fm1*)
    | KDuration (fm, tm) ->
        let tag : idx_ct_fm_disj_ex =
          KDuration
    ...
    | KFormula fm ->
        let tag : idx_ct_fm_disj_ex =
          KFormula (fm_disj_to_fm_disj_ex (fm_disj_of_fm fm))
        in
...
let fm_to_fm_disj_ex (fm : fm) : fm_disj_ex =
  ...
  fm_disj_to_fm_disj_ex
    (fm_disj_of_fm (fm_of_map_fm_disj_ex (fm_to_fm_disj_ex_map fm)))
...
let simplify (rmtld_formula : fm) : fm =
  ...
  let isol f lst =
    List.fold_left
      (fun (nineq, wineq) a ->
        if try f a ; false with _ -> true then (nineq, a :: wineq)
        else (a :: nineq, wineq) )
      ([], []) lst
  in
  let isol_conj (fm : fm_conj_ex) : fm_atom_ex list * fm_atom_ex list =
    isol
      (fm_atom_notless_of_fm_atom % fm_atom_of_fm_atom_ex)
      (fm_atom_ex_lst_of_fm_conj_ex fm)
  in
  let isol_disj (fm : fm_disj_ex) : fm_conj_ex list * fm_conj_ex list =
    isol
      (fm_conj_notless_of_fm_conj % fm_conj_of_fm_conj_ex)
      (fm_conj_ex_lst_of_fm_disj_ex fm)
  in
   ...
   *)
  let axiom1_primitive pval (fm1 : fm_disj_ex) (fm_r : fm_atom_ex)
      (fm2 : fm_disj_ex) (fm3 : fm_disj_ex) : fm_disj_ex =
    (* Axiom 1 - phi1 or ( phir and phi2 ) U phi3 -> [ ( phir -> (phi1 or phi2) U phi3 ) and (not phir -> phi1 U phi3)] *)
    let untilA : fm_atom_ex = ULess (pval, tc_disj fm1 fm2, fm3) in
    let untilB : fm_atom_ex = ULess (pval, fm1, fm3) in
    (* 
        DNF | (phir -> untilA) ∧ (¬phir -> untilB) ==> (untilB ∧ ¬phir) ∨ (phir ∧ untilA)
    *)
    tc_disj
      (`Conj (tc_conj (`X untilB) (`X (Not fm_r))))
      (`Conj (tc_conj (`X fm_r) (`X untilA)))
  in
  let axiom2_primitive pval (fm3 : fm_disj_ex) (fm_r : fm_atom_ex)
      (fm2 : fm_disj_ex) (fm1 : fm_disj_ex) : fm_disj_ex =
    (* Axiom 2 - phi1 U ((phir and phi2) or phi3) -> (phir -> phi1 U (phi2 or phi3)) and (not phir -> phi1 U phi3)) *)
    let untilA : fm_atom_ex = ULess (pval, fm1, tc_disj fm2 fm3) in
    let untilB : fm_atom_ex = ULess (pval, fm1, fm3) in
    tc_disj
      (`Conj (tc_conj (`X untilB) (`X (Not fm_r))))
      (`Conj (tc_conj (`X fm_r) (`X untilA)))
  in
  let apply_axiom axiom_primitive pval (dnf_fm : fm_disj_ex)
      (lst_dnf_wineq : fm_conj_ex list) (fm : fm_disj_ex) : fm_disj_ex =
    ...
    if fm_wineq_lst <> [] then
      let el, lst = select fm_wineq_lst in
      let fm_nineq_lst = fm_nineq_lst @ lst in
      let fm_wineq, fm_nineq =
        (el, `Conj (fm_conj_ex_of_fm_atom_ex_lst fm_nineq_lst))
      in
      axiom_primitive pval
        (tc_disj dnf_fm (fm_disj_ex_of_fm_conj_ex_lst lst_dnf_wineq_remain))
        fm_wineq fm_nineq fm
    else raise (Empty_wineq_lst ())
    (* ###### CONFIRM THIS LINE *)
  in
  ...
  in
  let join_map_fm map_fm1 map_fm2 =
    Fm_container.fold
      (fun key value map -> Fm_container.add key value map)
      map_fm1 map_fm2
  in
  let rec is_fm_disj_ex_solved (fm : fm_disj_ex) unsol_map sol_map =
    ...
    match fm with
    | `Or (cnj, dsj) ->
        is_fm_conj_ex_solved cnj unsol_map sol_map
        && is_fm_disj_ex_solved dsj unsol_map sol_map
  ...
  and is_fm_atom_ex_solved (fm : fm_atom_ex) unsol_map sol_map =
    ...
    | Prop id -> true
    | Less (tm1, tm2) ->
        is_tm_disj_ex_solved tm1 unsol_map sol_map
        && is_tm_disj_ex_solved tm2 unsol_map sol_map
    | Equal (tm1, tm2) ->
        is_tm_disj_ex_solved tm1 unsol_map sol_map
        && is_tm_disj_ex_solved tm2 unsol_map sol_map
    | EqualD (id, (tm, fm)) ->
        is_tm_disj_ex_solved tm unsol_map sol_map
        && is_fm_disj_ex_solved fm unsol_map sol_map
    | E (id, fm) -> is_fm_disj_ex_solved fm unsol_map sol_map
    | RU id -> (
      try
        Fm_container.find (Sexp.to_string (sexp_of_kmap (KProp id))) sol_map ;
        true
      with Not_found -> false )
    | _ ->
        raise
          (Failure
             "is_fm_atom_ex_solved is applied to unreplaced temporal operators")
  and is_fm_conj_ex_solved (fm : fm_conj_ex) unsol_map sol_map =
    match fm with
    | `And (at, cnj) ->
        is_fm_atom_ex_solved at unsol_map sol_map
        && is_fm_conj_ex_solved cnj unsol_map sol_map
  ...
  and is_tm_disj_ex_solved (tm : tm_disj_ex) unsol_map sol_map =
    ...
    | RD id -> (
      try
        Fm_container.find (Sexp.to_string (sexp_of_kmap (KVar id))) sol_map ;
        true
      with Not_found -> false )
    | _ ->
        raise
          (Failure
             "is_tm_disj_ex_solved is applied to unreplaced duration terms")
  in
  ...
  let heuristic dnf_map =
    let solve_isolation key (tuple : idx_ct_fm_disj_ex)
        (unsolved_map_fm, solved_map_fm) =
      (* to isolate until operators *)
      let isol_until_oper pval (fm1 : fm_disj_ex) (fm2 : fm_disj_ex) :
          idx_ct_fm_disj_ex =
        ...
        if lst_dnf_wineq <> [] then
          if lst_dnf_nineq <> [] then
            KFormula
              (apply_axiom axiom1_primitive pval
                 (fm_disj_ex_of_fm_conj_ex_lst lst_dnf_nineq)
                 lst_dnf_wineq fm2)
          else
            KFormula
              (apply_axiom axiom1_primitive pval
                 (`Conj (`X True))
                 lst_dnf_wineq fm2) (* ###### CONFIRM THIS LINE *)
        else
          ...
          if lst_dnf_wineq2 <> [] then
            if lst_dnf_nineq2 <> [] then
              KFormula
                (apply_axiom axiom2_primitive pval
                   (fm_disj_ex_of_fm_conj_ex_lst lst_dnf_nineq2)
                   lst_dnf_wineq2 fm1)
            else
              KFormula
                (apply_axiom axiom2_primitive pval
                   (`Conj (`X True))
                   lst_dnf_wineq2 fm1) (* ###### CONFIRM THIS LINE *)
          else KUntil (pval, fm1, fm2)
      in
      ...
      | KUntil (pval, fm1, fm2) ->
          ...
          if
            is_fm_disj_ex_solved fm1 unsolved_map_fm solved_map_fm
            && is_fm_disj_ex_solved fm2 unsolved_map_fm solved_map_fm
          then (
            verb_m 2 (fun _ ->
                print_endline
                  ( "fm1: "
                  ...
                  ^ "\n" ) ) ;
            let out_fm =
              isol_until_oper pval fm1 fm2
              (* let us begin by isolating this until operator *)
            in
            verb_m 2 (fun _ ->
                print_endline
                  ( "isol_until_oper: "
                  ^ Sexp.to_string (sexp_of_idx_ct_fm_disj_ex out_fm)
                  ^ "\n" ) ) ;
            let unsolved_fm_x =
              replace_fm key out_fm
      ...
      | KDuration (fm, tm) ->
          ...
          if
            is_fm_disj_ex_solved fm unsolved_map_fm solved_map_fm
            && is_tm_disj_ex_solved tm unsolved_map_fm solved_map_fm
          then (* TODO HERE *)
            (unsolved_map_fm, solved_map_fm)
          else (unsolved_map_fm, solved_map_fm)
      | KFormula fm ->
          (* if formula is solved then mark as solved else continue solving *)
          if is_fm_disj_ex_solved fm unsolved_map_fm solved_map_fm then
            ...
            (Fm_container.add key (KFormula fm) unsolved_map_fm, solved_map_fm)
      (* | _ -> raise (Failure ("simplify_heuristic applied to unknown tokens")) *)
    in
    let rec solve depth dnfmap =
      (* depth: max depth for search *)
      if dnfmap = Fm_container.empty then
        raise (Failure "simplify_heuristic of an empty formula")
      else
        let unsolved_fm, solved_fm =
          Fm_container.fold solve_isolation dnfmap
            (Fm_container.empty, Fm_container.empty)
        in
        if unsolved_fm = Fm_container.empty then solved_fm
        else if depth > 0 then solve (depth - 1) dnfmap
        else raise (Failure "heuristic solver exits with unsolved formulas")
    in
    solve 1000 dnf_map
    (* max depth fixed to 1000 iterations *)
  in
  try
    ...
    verb_m 1 (fun _ ->
        print_endline "Type conversion from fm to fm_disj_ex done.\n" ;
        print_fm_map_ex map_ex ) ;
    (* simplify using heuristic *)
    let fm_map_solved = heuristic map_ex in
    verb_m 1 (fun _ ->
        print_endline "Simplifcation using heuristic done.\n" ;
