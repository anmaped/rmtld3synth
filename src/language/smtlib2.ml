...
let set_option lst a b =
  lst := Statement.set_option ( Term.colon 
                                  (Term.const (Id.mk Id.Attr a))
                                  (Term.const (Id.mk Id.Term b))
                              ) :: !lst

let set_info lst a b =
  lst := Statement.set_info ( Term.colon 
                                (Term.const (Id.mk Id.Attr a))
                                (Term.const (Id.mk Id.Term b))
                            ) :: !lst

let set_logic lst a =
  lst := Statement.set_logic a :: !lst
...
let define_datatypes lst a =
  lst := Statement.datatypes a :: !lst

let check_sat lst =
  lst := Statement.check_sat [] :: !lst

let get_model lst =
  lst := Statement.get_model () :: !lst

let get_info lst ifo =
  lst := Statement.get_info ifo :: !lst

let declare_const lst id typ =
  lst := Statement.decl id typ :: !lst

let declare_fun lst id l typ =
  lst := Statement.fun_decl id l typ :: !lst

let assert_ lst a =
  lst := Statement.assert_ a :: !lst

let f_equal a b =
  Term.apply (Term.const (Id.mk Id.Term "=")) [a;b]

let ite a b c =
  Term.apply (Term.const (Id.mk Id.Term "ite")) [a;b;c]

let f_const_term a =
  (Term.const (Id.mk Id.Term a))

let f_const_sort a =
  (Term.const (Id.mk Id.Sort a))

let f_or a b =
  Term.apply (Term.const (Id.mk Id.Term "or")) [a;b]

let f_and a b =
  Term.apply (Term.const (Id.mk Id.Term "and")) [a;b]
  ...
  let evalfold id =
    ...
    if !recursive_unrolling then
      (
        ...
        let lst_all_comb = cartesian enumeration enumeration
        in
        List.fold_left
          (fun a (x,i) ->
             if x > i then
                   ...
                   (Term.apply
                      ...
                      [
                        ...
                        (Term.apply
                           ...
                           [
                             (f_minus (Term.const (Id.mk Id.Term (string_of_int x) )) (Term.const (Id.mk Id.Term "1")) );
                             (Term.const (Id.mk Id.Term (string_of_int i) ))
                           ]
                        )
                      ]
   ...
   (*
        ...
        assert_ lst (
          (Term.forall
             ([
               Term.colon (Term.const (Id.mk Id.Term "x")) (Term.const (Id.mk Id.Sort "Time"));
               Term.colon (Term.const (Id.mk Id.Term "i")) (Term.const (Id.mk Id.Sort "Time"))
             ])
             (ite 
                (f_less (Term.const (Id.mk Id.Term "i")) (Term.const (Id.mk Id.Term "x")))
                (f_equal
                   (Term.apply
                      (Term.const (Id.mk Id.Term ("evalb" ^ id) ) )
                      [
                        ...
                        (Term.apply
                           (Term.const (Id.mk Id.Term ("evalfold" ^ id) ) )
                           [
                             (f_minus (Term.const (Id.mk Id.Term "x")) (Term.const (Id.mk Id.Term "1")) );
                             (Term.const (Id.mk Id.Term "i"))
                           ]
                        )
                      ]
...
let synth_tm_duration (tm_call,tm_body) (fm_call,fm_body) helper =
  ...
  let duration id dt fm = (* trc_size >= t + dt is the worst case! (more improvments in the future are possible) *)
    let l_lst = ref [] in
        ...
        (
          ...
          in List.fold_left (fun a (x,i) ->
              if x > i then
                ...
                assert_ l_lst (f_equal
                                 (Term.apply (f_const_term ("evaleta" ^ id) ) ([(f_const_term (string_of_int x) );(f_const_term (string_of_int i) )]) )
                                 (f_sum
                                    (Term.apply
                                       (f_const_term ("evaleta"^ id) )
                                       ([
                                         (f_minus (f_const_term (string_of_int x) ) (f_const_term "1") ) ;
    ...
    (*
          ...
          assert_ l_lst (Term.forall
                           ...
                           (f_implies
                              ...
                              (f_geq (f_const_term "x") (f_const_term "0") ) (* ignores upper bound *)
                              (ite
                                 (* (and (and (< x (+ "^ (string_of_int t) ^" "^ dt ^") ) (>= i 0)) (> x i)) *) (* ignores upper bound *)
                                 (f_and (f_geq (f_const_term "i") (f_const_term "0") ) (f_less (f_const_term "i") (f_const_term "x") ))
                                 (* (= (evaleta"^ id ^" x i) (+ (evaleta"^ id ^" (- x 1) i) (indicator"^ id ^" trc x) )) *)
                                 (f_equal
                                    (f_sum
                                       (Term.apply
  ...
  (*
    ...
    define_fun l_lst
      ...
      (* body *)
      (f_and
         (f_geq (Term.const (Id.mk Id.Term "trc_size" )) (Term.const (Id.mk Id.Term "mt" )) )
         (Term.apply
            (Term.const (Id.mk Id.Term ("evaleta" ^ id ) ))
            ([
              (f_minus (Term.const (Id.mk Id.Term "mt" )) (Term.const (Id.mk Id.Term "1" )) );
              (Term.const (Id.mk Id.Term "mtb" ))
            ])
         )
      ) ;
  ...
  let dur_out1, dur_out2 = duration ("!"^(string_of_int idx)) tm_call fm_call in
...
let synth_tm_times cmptr1 cmptr2 helper =
  (Term.apply (Term.const (Id.mk Id.Term "*")) [fst cmptr1;fst cmptr2], snd cmptr1@snd cmptr2)

let synth_fm_true helper =
  empty_body (Term.const (Id.mk Id.Term "TVTRUE"))

let synth_fm_p p helper =
  empty_body (Term.apply (Term.const (Id.mk Id.Term "ev_prop")) [(Term.const (Id.mk Id.Term "mk")); (Term.const (Id.mk Id.Term "mt")); (Term.const (Id.mk Id.Term (string_of_int p) )) ])
...
let synth_fm_or cmpfm1 cmpfm2 helper =
  (Term.apply (Term.const (Id.mk Id.Term "tvor")) [fst cmpfm1;fst cmpfm2], snd cmpfm1@snd cmpfm2)

let synth_fm_less cmptr1 cmptr2 helper =
  (Term.apply (Term.const (Id.mk Id.Term "tvlessthan")) [fst cmptr1;fst cmptr2], snd cmptr1@snd cmptr2)
...
let synth_fm_uless gamma sf1 sf2 helper =
  ...
  let compute_until_less id (comp1, comp1_append) (comp2, comp2_append) =
   ...
   (*
      ...
      define_fun lst
        ...
        (* body *)
        (ite
           (f_equal (Term.const (Id.mk Id.Term "v")) (Term.const (Id.mk Id.Term "FVSYMBOL")) )
           (Term.apply
              (Term.const (Id.mk Id.Term "evali"))
              [
                (comp1);
                (comp2)
              ]
   ...
   (*
      ...
      define_fun lst
        ...
        (* return type *)
        (Term.apply (Term.const (Id.mk Id.Sort "Pair")) [(Term.const (Id.mk Id.Sort "Bool"));(Term.const (Id.mk Id.Sort "Fourvalue"))])
        (* body *)
        (Term.apply
           (Term.const (Id.mk Id.Term "mk-pair"))
           [
             (f_geq (f_sum (Term.const (Id.mk Id.Term (string_of_int gamma) )) (Term.const (Id.mk Id.Term "mtb")) ) (Term.const (Id.mk Id.Term "trc_size")) );
             (Term.apply
                (Term.const (Id.mk Id.Term ("evalfold" ^ id) ))
                [
                  (f_minus (Term.const (Id.mk Id.Term "mt" )) ((Term.const (Id.mk Id.Term "1" ))) );
  ...
  (
    Term.apply
      ...
      ([
        (f_sum (Term.const (Id.mk Id.Term ("mt"))) (Term.const (Id.mk Id.Term (  string_of_int (int_of_float gamma)  ))) );
        (Term.const (Id.mk Id.Term ("mt")) )
      ]),
    [] (* things are append in compute_until_less; we don't need (snd sf1@snd sf2) *)
  )

let synth_fm_ulesseq gamma sf1 sf2 helper = ignore( raise (Failure ("Unimplemented Feature"))) ; dummy_tuple

let synth_fm_ueq gamma sf1 sf2 helper = ignore( raise (Failure ("Unimplemented Feature"))) ; dummy_tuple (* this is similar to synth_fm_uless \/ synth_fm_ulesseq *)



let synth_smtlib_header () =
  let common_header () =
    set_info lst ":smt-lib-version" "2.6";
  in

  let common_header_cvc4 () =
    if !recursive_unrolling then set_logic lst "QF_AUFDTNIRA" else set_logic lst "AUFDTNIRA";
...
let synth_smtlib_common_types () =
  ...
  define_sort lst (Id.mk Id.Sort "Proptype") [] (Term.const (Id.mk Id.Sort "Int"));
  define_sort lst (Id.mk Id.Sort "Time") [] (Term.const (Id.mk Id.Sort "Int"));
  define_sort lst (Id.mk Id.Sort "Duration") [] (Term.const (Id.mk Id.Sort "Time"));
  ...
  define_sort lst (Id.mk Id.Sort "Trace") [] (Term.apply (Term.const (Id.mk Id.Sort "Array")) [Term.const (Id.mk Id.Sort "Time"); Term.const (Id.mk Id.Sort "Proptype");]);
 ...
 (*
  ...
  define_fun lst
    ...
    (* body *)
    (ite
       (f_equal (Term.const (Id.mk Id.Term "phi")) (Term.const (Id.mk Id.Term "TVTRUE")))
       (Term.const (Id.mk Id.Term "TVFALSE"))
       (ite
          (f_equal (Term.const (Id.mk Id.Term "phi")) (Term.const (Id.mk Id.Term "TVFALSE")))
 ...
 (*
  ...
  define_fun lst
    ...
    (* body *)
    (ite
       (f_or (f_equal (Term.const (Id.mk Id.Term "phi1")) (Term.const (Id.mk Id.Term "TVTRUE"))) (f_equal (Term.const (Id.mk Id.Term "phi2")) (Term.const (Id.mk Id.Term "TVTRUE"))) )
       (Term.const (Id.mk Id.Term "TVTRUE"))
       (ite
          (f_and (f_equal (Term.const (Id.mk Id.Term "phi1")) (Term.const (Id.mk Id.Term "TVFALSE"))) (f_equal (Term.const (Id.mk Id.Term "phi2")) (Term.const (Id.mk Id.Term "TVFALSE"))) )
          (Term.const (Id.mk Id.Term "TVFALSE"))
          (Term.const (Id.mk Id.Term "TVUNKNOWN"))
       )
    );
 ...
 (*
  ...
  define_fun lst
    ...
    (* body *)
    (ite
       (f_less (Term.const (Id.mk Id.Term "eta1")) (Term.const (Id.mk Id.Term "eta2")) )
       (Term.const (Id.mk Id.Term "TVTRUE"))
       (Term.const (Id.mk Id.Term "TVFALSE"))
    );
  ()
...
let synth_smtlib_common_macros () =
 ...
 (*
  ...
  define_fun lst
    ...
    (* body *)
    (ite
       (f_equal (Term.apply (Term.const (Id.mk Id.Term "second")) [(Term.const (Id.mk Id.Term "p"))]) (Term.const (Id.mk Id.Term "FVTRUE")))
       (Term.const (Id.mk Id.Term "TVTRUE"))
       (ite
          (f_equal (Term.apply (Term.const (Id.mk Id.Term "second")) [(Term.const (Id.mk Id.Term "p"))]) (Term.const (Id.mk Id.Term "FVSYMBOL")))
          (ite
             (Term.apply (Term.const (Id.mk Id.Term "first")) [(Term.const (Id.mk Id.Term "p"))])
             (Term.const (Id.mk Id.Term "TVUNKNOWN"))
             (Term.const (Id.mk Id.Term "TVFALSE"))
          )
          (ite
             (f_equal (Term.apply (Term.const (Id.mk Id.Term "second")) [(Term.const (Id.mk Id.Term "p"))]) (Term.const (Id.mk Id.Term "FVFALSE")))
             (Term.const (Id.mk Id.Term "TVFALSE"))
             (Term.const (Id.mk Id.Term "TVUNKNOWN"))
          )
       )
...
let synth_smtlib_common_evali () =
 (*
  ...
  define_fun lst
    ...
    (* body *)
    (ite
       (f_equal (Term.const (Id.mk Id.Term "b2")) (Term.const (Id.mk Id.Term "TVFALSE")))
       (ite
          (f_equal (Term.const (Id.mk Id.Term "b1")) (Term.const (Id.mk Id.Term "TVTRUE")))
          (Term.const (Id.mk Id.Term "FVSYMBOL"))
          (Term.apply (Term.const (Id.mk Id.Term "mapb4")) [(Term.const (Id.mk Id.Term "b1"))])
       )
       (Term.apply (Term.const (Id.mk Id.Term "mapb4")) [(Term.const (Id.mk Id.Term "b2"))])
    );
  ()
...
let synth_smtlib_common_trace () =
 (*
  ...
  declare_const lst (Id.mk Id.Term "trc_size") (Term.const (Id.mk Id.Sort "Time"));
  ()
 ...
 (*
  ...
  define_fun lst
    ...
    (ite
       (f_geq (Term.const (Id.mk Id.Term "trc_size")) (Term.const (Id.mk Id.Term "mt")) )
       (ite
          (f_equal (Term.apply (Term.const (Id.mk Id.Term "select")) [(Term.const (Id.mk Id.Term "mk")); (Term.const (Id.mk Id.Term "mt"))]) (Term.const (Id.mk Id.Term "p")) )
          (Term.const (Id.mk Id.Term "TVTRUE"))
          (Term.const (Id.mk Id.Term "TVFALSE"))
       )
       (Term.const (Id.mk Id.Term "TVUNKNOWN"))
...
let synth_smtlib synth_fun formula helper =
  ...
  let tm,stm = synth_fun formula helper
  in add_l stm;
  ...
  if (isZ3SolverEnabled () ) then
    assert_ lst (Term.forall
                   ([
                     Term.colon (Term.const (Id.mk Id.Term "t")) (Term.const (Id.mk Id.Sort "Time"))
                   ])
                   (f_geq
                      (Term.apply
  ...
  add (Statement.assert_ (Term.apply (Term.const (Id.mk Id.Term "allcheck") ) [(Term.const (Id.mk Id.Term "trc")); (Term.const (Id.mk Id.Term "0"))] ) );
  ...
  List.iter (fun a -> Statement.print Format.str_formatter a; Format.pp_print_newline (Format.str_formatter) ()) (List.rev !lst);
